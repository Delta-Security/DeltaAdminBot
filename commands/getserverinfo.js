...
module.exports.run = async (client, message, args) => {
  ...
  .setFooter('API Latency is ' + `${Date.now() - message.createdTimestamp}` + ' ms, Proudly Hosted by DeltaSec', client.user.avatarURL()L)
  .setTimestamp()
	...
	con.query(`SELECT * FROM users WHERE discordid = '${userid}'`, function (err, result, fields) { // Basic MySQL Query to select everything from users where discordis = the author of the message (ID).
	        ...
	        if(rows.api_type == "Client" || rows.api_type == "client") { // If API_type is Client or client
				...
				let embed = new Discord.MessageEmbed()
				    ...
				    .setTimestamp()
		    		.setFooter('API Latency is ' + `${Date.now() - message.createdTimestamp}` + ' ms, Proudly Hosted by DeltaSec', client.user.avatarURL()L); // Ping
		    		/* Sends the message */
