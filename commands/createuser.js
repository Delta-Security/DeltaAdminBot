...
module.exports.run = async (client, message, args) => {
  ...
  con.query(`SELECT * FROM users WHERE discordid = '${userid}'`, function (err, result, fields) {
          ...
          .setFooter('API Latency is ' + `${Date.now() - message.createdTimestamp}` + ' ms, Proudly Hosted by DeltaSec', client.user.avatarURL())
          .setTimestamp()
          .setDescription("❎ | You are not registered into our systems! Please register at **http://deltasec.systems/register.php** then try again") 
         ...
         let embed1 = new Discord.MessageEmbed()
            ...
            .setTimestamp()
            .setFooter('API Latency is ' + `${Date.now() - message.createdTimestamp}` + ' ms, Proudly Hosted by DeltaSec', client.user.avatarURL())
            .setDescription("```" + `Heyy ${message.author.tag}👋! Thank you for using DeltaSecAdmin. I will be guiding you during the setup of your account😉. So lets get started❤️` + "```")
